    ...
    protected function modify() {
        try {
            ...
            if (strtolower($source) == 'options') {
                ...
                $alpha = strtolower($alpha[0]);
                
                $business_info = $this->model->select("business_".$alpha, '*', array('md5(sno)'=>$company), NULL, 1);
                ...
                if (empty($data))
                {
                    ...
                    $this->data['results'] = $business_info;
                    
                    $this->data['alpha'] = $alpha;

                    $this->page = 'company/edit_business'; 
                    return;
                    ...
                    $imagelogo = BASEDIR . "/assets/uploads/".$alpha."_".$business_info->sno."/logo" . $business_info->logo;
                ...
                if (isset($files['logo']))
                {
                    $dir = "assets/uploads/".$alpha."_".$business_info->sno."/logo";
                    //echo $dir;
                    $this->imageuploader->
                            set('file', $files)->
                            set('name', 'logo')->
                            ...
                            set('fileType', 'image')->
                            set('root', BASEDIR)->
                            set('directory', "assets/uploads/".$alpha."_".$business_info->sno."/logo")->
                            set('minWidth', 150)->
                            set('minHeight', 150);
                ...
                if($image_upload){
                        ...
                        $post_data = array(
                            ...
                            'logo'=>$this->imageuploader->get('fileName')
                            
                        );
                        $post_update = $this->model->update("business_$alpha", $post_data, array('md5(sno)' => $company));

                        $uri = base_url() . "superadmin/?tab=company&target=view&source=tab&alpha=$alpha";
                        //echo $uri; return; 
                        redirect($uri);
                    }
                }
